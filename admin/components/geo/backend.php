...
//=================================================================================================//
	...
	if ($opt == 'saveorder'){
        ...
        cmsCore::addSessionMessageAdmin($_LANG['AD_DO_SUCCESS'], 'success');
        cmsCore::redirectBack();
...
//=================================================================================================//
	...
	if($opt=='saveconfig'){
		...
		cmsCore::addSessionMessageAdmin($_LANG['AD_CONFIG_SAVE_SUCCESS'], 'success');
...
if ($opt == 'show_district'){
    if (!isset($_REQUEST['item'])){
        if (isset($_REQUEST['item_id'])){ dbShow('cms_geo_districts', (int)$_REQUEST['item_id']); }
        echo '1'; exit;
    }
}
...
if ($opt == 'hide_district'){
    if (!isset($_REQUEST['item'])){
        if (isset($_REQUEST['item_id'])){ { dbHide('cms_geo_districts', (int)$_REQUEST['item_id']); } }
        echo '1'; exit;
    }
}
